module EventMachine
  ...
  class Connection
    ...
    # {#ssl_handshake_completed},
    #
    # When this method is overridden at all, it is recommended that
    # `preverify_ok` be returned unchanged and only use this callback for
    # logging and diagnostics.  If OpenSSL isn't verifying a certificate, you
    # should first try setting {SSL::SSLContext#ca_path} or
    # {SSL::SSLContext#ca_file}, ensuring SNI {hostname} was set, etc.
    #
    ...
    #   error per certificate, so it may be called multiple times with the same
    #   error depth.
    #
    ...
    #
    # @option args [SSL::SSLContext] :context (nil)  The SSL context configuration.
    #   {EM::SSL::SSLContext#setup} will be called, which freezes the context.
    #
    ...
    #   {EM::SSL::SSLContext}.  If :context is provided, any extra options will
    ...
    def start_tls args={}
      ...
      if hostname && sni_hostname
        ...
        context = EM::SSL::SSLContext.new
        context.set_params(args)
      end

      @ssl_connection = EM::SSL::SSLConnection.new(self, context)
      ssl_connection.hostname   = hostname || sni_hostname
      ssl_connection.sync_close = sync_close
      ssl_connection.start_tls
    end

    # @return [EM::SSL::SSLConnection] the SSL connection, after calling start_tls
